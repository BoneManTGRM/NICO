...
const workerTools = ["pip-audit", "npm-audit", "osv-scanner", "semgrep", "bandit", "eslint", "pytest", "npm-test", "npm-build"];
...
export default function Page() {
  ...
  const [selectedWorkerTools, setSelectedWorkerTools] = useState<string[]>(["pip-audit", "npm-audit", "osv-scanner", "bandit"]);
  ...
  async function loadApprovals() {
    ...
    setApprovalError("");
    try { const response = await fetch(`${API_URL}/approvals`, {cache: "no-store"}); const data = await response.json(); if (!response.ok) throw new Error(`Approvals failed with ${response.status}`); setApprovals(Array.isArray(data) ? data : []); }
  ...
  async function transitionApproval(approvalId: string, action: "approve" | "reject" | "needs-more-evidence") {
    if (!backendConfigured) return;
    setApprovalError("");
    try { const response = await fetch(`${API_URL}/approvals/${approvalId}/${action}`, {method: "POST", headers: {"Content-Type": "application/json"}, body: JSON.stringify({actor: "frontend_reviewer", note: "Updated from NICO frontend"})}); if (!response.ok) throw new Error(`Approval update failed with ${response.status}`); await loadApprovals(); }
