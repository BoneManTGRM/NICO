...
const assessmentAreas = ["Code Audit", "Dependency / Library Ecosystem", "Secrets Exposure Review", "Static Analysis", "CI/CD Analysis", "Architecture & Technical Debt", "Velocity / Complexity", "QA / Functional Review", "Platform Parity", "Retainer Ops", "Markdown / HTML / PDF Reports"];

const targetCards = [
  ["Express Technical Health Assessment", "90–95%", "Highest automation target after scanner/report upgrades"],
  ["Mid Technical Health Assessment", "75–85%", "QA, parity, stakeholder intake, and roadmap evidence"],
  ["Ongoing Product Engineering Retainer", "55–70%", "Backlog, sprint, release, and status-report workflows"],
  ["Full client-ready replacement", "75–85%", "Human validation before client delivery"],
];
...
type AssessmentResult = {status?: string; repository?: string; generated_at?: string; executive_summary?: string; maturity_signal?: {level?: string; score?: number; summary?: string}; maturity_semaphore?: Record<string, string>; sections?: Section[]; findings?: string[]; repairs?: string[]; reports?: {markdown?: string; html?: string; pdf_base64?: string; pdf_filename?: string}; safety_boundary?: string; human_review_required?: boolean};
type WorkflowResult = {status?: string; workflow?: string; target_coverage?: string; generated_at?: string; maturity_signal?: {level?: string; score?: number; summary?: string}; sections?: Section[]; qa_checklist?: string[]; parity_checklist?: string[]; six_month_roadmap?: string[]; weekly_status_report?: string[]; monthly_strategy_report?: string[]; release_checklist?: string[]; human_approval_queue?: string[]; human_review_required?: boolean};
...
export default function Page() {
  ...
  async function runHostedAssessment() {
    ...
    try {
      const response = await fetch(`${API_URL}/assessment/github`, {method: "POST", headers: {"Content-Type": "application/json"}, body: JSON.stringify({repository, authorized, client_name: clientName, project_name: projectName, assessment_mode: "express", timeframe_days: 180})});
      const data = await response.json();
      if (!response.ok) throw new Error(data?.detail?.error || data?.error || `Assessment failed with ${response.status}`);
      setAssessment(data);
    } catch (error) { setAssessmentError(error instanceof Error ? error.message : "Assessment failed"); } finally { setLoading(false); }
  }
  ...
  async function runMidWorkflow() {
    ...
    try {
      const response = await fetch(`${API_URL}/assessment/mid`, {method: "POST", headers: {"Content-Type": "application/json"}, body: JSON.stringify({authorized, client_name: clientName, project_name: projectName, qa_evidence: extractBlock(midNotes, "QA evidence"), parity_notes: extractBlock(midNotes, "Parity notes"), stakeholder_notes: extractBlock(midNotes, "Stakeholder notes"), roadmap_notes: extractBlock(midNotes, "Roadmap notes"), known_risks: extractBlock(midNotes, "Known risks")})});
      const data = await response.json();
      if (!response.ok) throw new Error(data?.detail?.error || data?.error || `Mid workflow failed with ${response.status}`);
      setMidResult(data);
    } catch (error) { setOpsError(error instanceof Error ? error.message : "Mid workflow failed"); } finally { setLoading(false); }
  }
  ...
  return (
    <main className="shell">
      <section className="hero"><p className="eyebrow">NICO Hosted Command Center</p><h1>Highest realistic assessment targets</h1><p className="lead">NICO now targets Express, Mid, and Retainer coverage with hosted backend workflows. Final client delivery still requires human review.</p><div className="hero-actions"><a href="#hosted" className="primary-link">Run Express</a><a href="#mid" className="secondary-link">Mid workflow</a><a href="#retainer" className="secondary-link">Retainer Ops</a></div></section>
      ...
      <section id="hosted" className="section panel"><div className="section-head"><div><p className="eyebrow">Express Assessment</p><h2>Assess an authorized GitHub repository</h2></div><span className="status gray">90–95%</span></div><p className="warning-box">Only assess repositories you own or are explicitly authorized to review. NICO performs defensive read-only assessment and does not make destructive changes.</p><div className="form-grid"><label>Repository owner/name or GitHub URL<input value={repository} onChange={(event) => setRepository(event.target.value)} placeholder="owner/repo" /></label><label>Client name, optional<input value={clientName} onChange={(event) => setClientName(event.target.value)} placeholder="Client name" /></label><label>Project name, optional<input value={projectName} onChange={(event) => setProjectName(event.target.value)} placeholder="Project name" /></label></div><label className="check-row"><input type="checkbox" checked={authorized} onChange={(event) => setAuthorized(event.target.checked)} />I confirm I own this target or have explicit permission to assess it.</label><button type="button" className="primary-button" disabled={!backendConfigured || !authorized || loading} onClick={runHostedAssessment}>{loading ? "Running..." : "Run Express assessment"}</button>{assessmentError ? <p className="error-box">{assessmentError}</p> : null}</section>
      ...
      <section id="commands" className="section panel"><div className="section-head"><div><p className="eyebrow">No-server fallback</p><h2>Run NICO from your local CLI</h2></div><span className="status blue">CLI ready</span></div><p className="muted">Use these commands on your computer or Codespace when you want local folder/archive/passive URL assessment.</p><div className="command-grid"><div className="command-card"><b>First test with NICO test lab</b><textarea readOnly defaultValue={quickStartCommands} /></div><div className="command-card"><b>Assess authorized systems locally</b><textarea readOnly defaultValue={assessmentCommands} /></div></div></section>
