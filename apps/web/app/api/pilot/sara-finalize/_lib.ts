...
function numbers(value: unknown, key: string): number[] {
  return collect(value, (candidate) => candidate === key)
    .map((item) => typeof item === "number" ? item : Number(item))
    .filter(Number.isFinite);
}

function booleans(value: unknown, key: string): boolean[] {
  return collect(value, (candidate) => candidate === key)
...
function pdfDigest(identity: JsonRecord): string {
  return String(asRecord(asRecord(identity.artifact_digests).pdf).sha256 || "")
    .trim()
    .toLowerCase();
}

export function assertExactRun(payload: JsonRecord): {identity: JsonRecord; pdfSha256: string} {
  ...
  const pdfSha256 = pdfDigest(identity);
  ...
  const unresolved = numbers(payload, "total_unresolved_human_review_work_units");
  if (!unresolved.length || unresolved.some((count) => count !== 0)) {
    throw new PilotActionError("The combined unresolved human-review workload is absent or not zero.", 409);
  }
  if (booleans(payload, "operator_attention_required").some(Boolean)) {
    throw new PilotActionError("NICO still reports unresolved operator attention.", 409);
  }
...
export function approvedPdf(payload: JsonRecord): {bytes: Uint8Array; filename: string; sha256: string} {
  ...
  if (!encoded) throw new PilotActionError("The approved response did not contain the final PDF.", 502);
  ...
  return {
    ...
    filename: safeFilename(reports.pdf_filename, `nico-comprehensive-${NICO_RUN_ID}-APPROVED-FINAL.pdf`),
    sha256,
  };
}

export async function approvedDeliveryPackage(token: string): Promise<{bytes: Uint8Array; filename: string; sha256: string}> {
  ...
  const expected = String(response.headers.get("x-nico-delivery-package-sha256") || "").trim().toLowerCase();
  ...
  try { decodedName = decodeURIComponent(rawName); } catch { /* Preserve the safe fallback below. */ }
  return {
    bytes,
...
export async function operatorTokenFromForm(request: NextRequest, confirmation: string): Promise<string> {
...
export function errorResponse(caught: unknown): Response {
  const error = caught instanceof PilotActionError
    ? caught
    : new PilotActionError(caught instanceof Error ? caught.message : "The protected action failed closed.", 500);
  return new Response([
