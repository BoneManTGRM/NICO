...
const COMPREHENSIVE_LOCALIZED_REPORT = /^\/assessment\/comprehensive-run\/[^/?#]+\/localized-report\/(?:en|es-MX)(?:\/(?:pdf|evidence-package))?$/;
  ...
  for (let attempt = 0; attempt < policy.retryDelaysMs.length; attempt += 1) {
    ...
    try {
      ...
      for (const name of [
        ...
        "x-nico-evidence-manifest-sha256",
        "x-nico-artifact-scope",
        "x-nico-run-revision",
