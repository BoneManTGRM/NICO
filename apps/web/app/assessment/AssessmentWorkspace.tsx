...
import type {Copy, Locale, Service} from "./assessmentTypes";
...
function DepthSelector({service, running, copy, choose}: {
  service: Service;
  running: boolean;
  copy: Copy;
  choose: (service: Service) => void;
}) {
  return <div className={styles.tierGrid} aria-label={copy.depthLabel}>
    {(["express", "comprehensive"] as Service[]).map((value) => <button
      type="button"
      key={value}
      className={service === value ? "primary-button" : "secondary-link"}
      aria-pressed={service === value}
      disabled={running}
      onClick={() => choose(value)}
      data-assessment-depth={value === "express" ? "core" : "strategic"}
      data-implementation-service={value}
    >
      <strong>{copy.services[value].label}</strong>
      <small>{copy.services[value].heading}</small>
    </button>)}
  </div>;
}

function ProgressTimeline({items, copy}: {
  items: ReturnType<typeof progressFor>;
  copy: Copy;
}) {
...
function Scorecard({sections, copy}: {
  sections: NonNullable<ReturnType<typeof assessmentFor>>["sections"];
  copy: Copy;
}) {
...
export default function AssessmentWorkspace({locale = "en"}: {locale?: Locale}) {
  ...
  function downloadPdf(): void {
    ...
    savePdf(report.pdf_base64, report.pdf_filename || `nico-${service === "express" ? "core" : "strategic"}-assessment.pdf`);
  }

  return <main className="shell" data-assessment-service-count="1" data-assessment-depth-count="2" data-assessment-locale={locale}>
    ...
    <section id="assessment" className="section panel">
      <div className="section-head">
        <div><p className="eyebrow">{serviceCopy.eyebrow}</p><h2>{serviceCopy.heading}</h2></div>
        <span className="status gray">{coverageLabel}</span>
      </div>
      <p className="summary-box">{serviceCopy.summary}</p>
      <DepthSelector service={service} running={running} copy={copy} choose={choose} />
      ...
      <button type="button" className="primary-button" disabled={!authorized || !repository.trim() || running} onClick={run}>
        {running ? `${copy.phases.running}: ${serviceCopy.label}` : `${copy.run} · ${serviceCopy.label}`}
      </button>
    ...
    <section className="section panel" aria-live="polite">
      <div className="section-head">
        <div><p className="eyebrow">{copy.state}</p><h2 title={result?.run_id}>{result?.run_id ? compactIdentifier(result.run_id, 18, 8) : copy.phases[phase]}</h2></div>
      ...
      {result ? <>
        <div className="grid four target-grid">
          ...
          <article><b>{copy.commit}</b><IdentifierValue value={immutableCommit} fallback={copy.notVerified} copy={copy} /></article>
        ...
        <div className="report-actions">
          <button type="button" disabled={!report?.markdown} onClick={copyMarkdown}>{copy.copy}</button>
          <button type="button" disabled={!report?.pdf_base64} onClick={downloadPdf}>{copy.download}</button>
          {copied ? <span className="muted">{copy.copied}</span> : null}
        </div>
