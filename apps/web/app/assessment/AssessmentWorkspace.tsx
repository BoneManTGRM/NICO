...
export default function AssessmentWorkspace({locale = "en"}: {locale?: Locale}) {
  ...
  const scannerStatus = service === "express"
    ...
    : stage(result, "dependency_security_static_analysis")?.status || stage(result, "deep_scanner_triage")?.status || copy.pending;
  ...
  return <main className="shell" data-assessment-service-count="2" data-assessment-locale={locale}>
    ...
    <section className="section panel" aria-live="polite">
      ...
      {result ? <>
        <div className="grid four target-grid"><article><b>{copy.runId}</b><span>{result.run_id || copy.notVerified}</span></article><article><b>{copy.commit}</b><span>{immutableCommit}</span></article><article><b>{copy.scanner}</b><span>{scannerStatus}</span></article><article><b>{copy.report}</b><span>{reportStatus}</span></article></div>
        <div className="grid four target-grid"><article><b>{copy.review}</b><span>{phase === "review_required" ? copy.phases.review_required : copy.pending}</span></article><article><b>{copy.maturity}</b><span>{assessment?.maturity_signal?.level || copy.pending}</span></article><article><b>{copy.score}</b><span>{scoreLabel}</span></article><article><b>{copy.durable}</b><span>{result.persistence?.durable ? copy.yes : result.persistence?.recorded ? copy.recorded : copy.notVerified}</span></article></div>
        {assessment?.executive_summary ? <p className="summary-box">{assessment.executive_summary}</p> : null}
        {progressItems.length ? <div className={styles.timeline}>{progressItems.map((item, index) => <article className="result-card" key={`${item.step}-${index}`}><div className="result-head"><b>{copy.stageLabels[String(item.step || "")] || String(item.step || copy.stage).replaceAll("_", " ")}</b><span className={statusClass(item.status)}>{item.status || copy.notVerified}</span></div><p>{item.message || copy.notVerified}</p>{item.evidence ? <details className="help-details"><summary>{copy.stepEvidence}</summary><pre className="json-block">{JSON.stringify(item.evidence, null, 2)}</pre></details> : null}</article>)}</div> : null}
        {assessment?.sections?.length ? <div className="results-grid">{assessment.sections.map((section, index) => { const value = section.presented_score ?? section.score; const label = typeof value === "number" ? `${value}/100` : copy.notScored; const state = section.presented_status || section.status || "unknown"; return <article className="result-card" key={section.id || index}><div className="result-head"><b>{section.label || section.id}</b><span className={statusClass(state)}>{state} · {label}</span></div><p>{section.summary}</p><details className="help-details"><summary>{copy.evidence} ({section.evidence?.length || 0})</summary><List items={section.evidence} empty={copy.notVerified} /></details>{section.findings?.length ? <details className="help-details"><summary>{copy.findings} ({section.findings.length})</summary><List items={section.findings} empty={copy.notVerified} /></details> : null}{section.unavailable?.length ? <details className="help-details"><summary>{copy.unavailable} ({section.unavailable.length})</summary><List items={section.unavailable} empty={copy.notVerified} /></details> : null}</article>; })}</div> : null}
        ...
        {assessment?.unavailable_data_notes?.length ? <details className="help-details"><summary>{copy.unavailable} ({assessment.unavailable_data_notes.length})</summary><List items={assessment.unavailable_data_notes} empty={copy.notVerified} /></details> : null}
      </> : null}
    </section>
  </main>;
}
