...
function reportReady(payload: JsonRecord, tier: "express" | "mid" | "full"): boolean {
  ...
  if (tier === "express") {
    return Boolean(reports.markdown || reports.html || reports.pdf_base64 || reports.report_id);
  }
  if (tier === "mid") {
...
function reconcileProgress(previous: JsonRecord | undefined, next: JsonRecord, state: RunState): JsonRecord {
  ...
  if (previous) {
    for (const field of ["repository_snapshot", "repository_evidence", "scanner", "scanner_evidence", "assessment", "reports", "mid_report", "approval", "approval_request", "persistence"] as const) {
...
function normalizeFinalGate(payload: JsonRecord, runId: string, state: RunState): JsonRecord {
  ...
  if (reportReady(payload, tier) && reviewReady(payload, tier)) {
    ...
    output.final_gate_reconciliation = {
      status: "completed_from_artifact_evidence",
      tier,
      report_ready: true,
      review_ready: true,
      source_status: status || "running",
    };
