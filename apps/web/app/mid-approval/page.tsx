...
type ApprovalResponse = {status?: string; approval?: Approval; detail?: {message?: string; validation?: Validation; missing_reviewed_item_ids?: string[]}};

function statusClass(value?: string) {
  const normalized = String(value || "").toLowerCase();
  if (["approved", "ready"].includes(normalized)) return "status green";
  if (["pending", "needs_more_evidence", "requested"].includes(normalized)) return "status yellow";
...
export default function MidApprovalPage() {
  ...
  const allItems = approval?.exception_item_ids || [];
  const allReviewed = useMemo(() => allItems.length === reviewed.length && allItems.every((item) => reviewed.includes(item)), [allItems, reviewed]);
  ...
  async function requestApproval() {
    ...
    try {
      ...
      setApproval(current);
      setReviewed(current?.review_decision?.reviewed_item_ids || []);
    } catch (caught) {
  ...
  async function refreshApproval() {
    ...
    try {
      ...
      setReviewed(data.approval?.review_decision?.reviewed_item_ids || reviewed);
    } catch (caught) {
  ...
  async function decide(state: "approved" | "needs_more_evidence" | "rejected") {
    ...
    try {
      const response = await fetch(`${API_URL}/assessment/mid-run/approval/${encodeURIComponent(approval.approval_id)}/${state}`, {
        method: "POST",
        headers: {"Content-Type": "application/json", "X-NICO-Admin-Token": adminToken},
        body: JSON.stringify({actor, note, reviewed_item_ids: reviewed}),
        cache: "no-store",
      });
      const data = await json(response);
      setApproval(data.approval || null);
  ...
  return <main className="shell">
    <section className="hero">
      ...
      <p className="lead">Approve only after reviewing every current exception bound to the exact run, snapshot, truth model, draft PDF, and review packet.</p>
    </section>
    ...
    {approval ? <>
      <section className="section panel">
        ...
        <div className="grid four target-grid">
          <article><b>Exception items</b><span>{approval.exception_item_count || 0}</span></article>
          <article><b>Ready for approval</b><span>{String(Boolean(approval.validation?.ready_for_approval))}</span></article>
      ...
      <section className="section panel">
        <div className="section-head"><div><p className="eyebrow">Exception acknowledgment</p><h2>Review every current item</h2></div><span className={allReviewed ? "status green" : "status yellow"}>{reviewed.length}/{allItems.length}</span></div>
        <label className="check-row"><input type="checkbox" checked={allReviewed} onChange={() => setReviewed(allReviewed ? [] : [...allItems])} />Acknowledge all current exception items</label>
        <div className="checkbox-grid">{allItems.map((item) => <label key={item}><input type="checkbox" checked={reviewed.includes(item)} onChange={() => toggleItem(item)} />{item}</label>)}</div>
        <div className="form-grid">
        ...
        <div className="report-actions">
          <button type="button" className="primary-button" disabled={!approval.validation?.ready_for_approval || !allReviewed || actor.trim().length < 2 || note.trim().length < 10 || loading || approval.status === "approved" || approval.status === "rejected"} onClick={() => decide("approved")}>Approve and generate separate PDF</button>
          ...
          <button type="button" disabled={actor.trim().length < 2 || note.trim().length < 5 || loading || approval.status === "approved" || approval.status === "rejected"} onClick={() => decide("rejected")}>Reject</button>
        </div>
      </section>
