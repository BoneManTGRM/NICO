...
The latest verified deployed main commit is `b8aaba855a68fd33fae0260db23b81ff01b062c6` (`Combine deployed browser and API production proof (#432)`). Its Vercel and Railway deployment checks passed. This deployment includes the collision-resistant Express run/report identity and exact-run persistence repair from PR #430, plus the protected browser-driven Express/Mid/Full production-proof workflow from PR #432. The workflow is now capable of producing one bounded evidence package from real deployed browser starts without duplicate API starts, while preserving exact-run identity, expected-origin checks, screenshots, human review, and `client_ready: false`. This deployment does not prove that any Express, Mid, or Full production assessment completed correctly. Deployed browser/API E2E proof remains incomplete until the protected production smoke is authorized, executed, retained, and reviewed.
