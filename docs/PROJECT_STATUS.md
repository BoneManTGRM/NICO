...
The latest verified deployed main commit is `1d1027ce0655133a14d2434538bd26423ae11f5e` (`Keep accepted Express runs alive through transient status outages (#439)`). Its Vercel and Railway deployment checks passed. The Express start remains strictly single-shot and returns the exact tenant-bound `express_run_*` identity introduced by PR #436. Read-only status continuation now tolerates up to eight consecutive mobile, LTE, proxy, or retryable backend transport interruptions with bounded exponential backoff, resets the failure count after any successful status response, and never creates a replacement assessment. Exact-run blocked, failed, rejected, interrupted, or other terminal evidence remains terminal and cannot be retried into a pass. Human review, `client_ready: false`, bounded capacity, duplicate-active-start prevention, and authenticated Recovery visibility remain enforced. This deployment does not prove that any Express, Mid, or Full production assessment completed correctly. Deployed browser/API E2E proof remains incomplete until the protected production smoke is authorized, executed, retained, and reviewed.
