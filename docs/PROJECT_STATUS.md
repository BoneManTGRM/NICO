...
The latest verified deployed main commit is `46651c337fd84025af07f8ea017a2fd8c64057ab` (`Fix interrupted Express assessments with exact-run polling (#436)`). Its Vercel and Railway deployment checks passed. This deployment replaces the fragile several-minute mobile browser-to-Railway Express request with one quick authorized lifecycle start, an exact tenant-bound `express_run_*` identity, and short same-origin status polling. It preserves the existing evidence/report pipeline, human review, `client_ready: false`, bounded capacity, duplicate-active-start prevention, fail-closed restart handling, and authenticated Recovery visibility for interrupted Express work. The protected browser/API production-proof workflow now requires exact-run polling for Express, Mid, and Full without issuing duplicate starts. This deployment does not prove that any Express, Mid, or Full production assessment completed correctly. Deployed browser/API E2E proof remains incomplete until the protected production smoke is authorized, executed, retained, and reviewed.
