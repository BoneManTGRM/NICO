...
| Add raw-secret regression tests for reports/API/UI state | tests/security | high | Prevents trust-damaging secret leakage | Current tests cover masking but should be expanded across report and API output | Add explicit no-raw-secret assertions across report files and API responses | Next hardening pass | No |
