...
ASSESSMENT_DIR = ROOT / "apps" / "web" / "app" / "assessment"
...
def test_missing_authorization_has_distinct_safe_guidance() -> None:
    result = {"status": "blocked", "error": "Explicit authorization is required before NICO assesses a repository."}
...
def test_invalid_repository_format_has_distinct_safe_guidance() -> None:
    detail = assessment_block_detail({"status": "blocked", "error": "repository must be owner/name or a GitHub repository URL"})
...
def test_unknown_policy_block_remains_generic_and_redacted() -> None:
    detail = assessment_block_detail({"status": "blocked", "code": "unrecognized_internal_reason", "error": "secret internal detail"})
    assert detail == {
        "status": "blocked",
