...
def test_action_plan_has_ordered_steps_for_empty_payload():
    result = build_max_target_action_plan({})

    assert result["ready_for_all_max"] is False
    assert result["step_count"] > 0
    assert result["first_step"]["service"] == "client_ready"


def test_action_plan_has_no_steps_when_ready():
    payload = {
    ...
    result = build_max_target_action_plan(payload)
