...
from nico.decision_grade_accepted_edition_v1 import build_accepted_report_edition
...
BASE = {
    ...
    "accepted_at": "2026-07-25T22:30:00+00:00",
}
...
def test_approved_complete_edition_allows_delivery() -> None:
    ...
    assert set(result["artifact_digests"]) == {"markdown", "html", "pdf", "json"}


def test_missing_artifact_blocks_approval() -> None:
    values = dict(BASE)
    values["artifacts"] = {key: value for key, value in BASE["artifacts"].items() if key != "pdf"}
...
def test_rejection_never_allows_delivery() -> None:
    values = dict(BASE, decision="rejected", decision_reason="Residual risk was not accepted.")
    result = build_accepted_report_edition(**values)
...
def test_missing_identity_and_reviewer_fail_closed() -> None:
    values = dict(BASE, commit_sha="", reviewer="")
    result = build_accepted_report_edition(**values)
