...
def test_spanish_production_proof_is_latest_release_only() -> None:
    ...
    assert "cancel-in-progress: false" not in source


def test_latest_release_policy_does_not_weaken_exact_release_or_delivery_gates() -> None:
    source = WORKFLOW.read_text(encoding="utf-8")
