...
def test_successful_review_mutation_rehydrates_exact_run_before_pdf_handoff() -> None:
    ...
    assert "const requireApprovedPdf = approvalRequested(init)" in HYDRATION
...
def test_existing_desktop_and_ios_pdf_handoff_remains_and_sits_lower() -> None:
    assert 'window.open("about:blank", "nico-comprehensive-pdf")' in HANDOFF
    assert "targetWindow.location.replace(href)" in HANDOFF
    assert 'data-final-review-pdf-handoff="ready"' in HANDOFF
    assert 'data-final-review-pdf-open="true"' in HANDOFF
    assert 'bottom: "max(8px, env(safe-area-inset-bottom))"' in HANDOFF
