...
def test_explicit_anonymous_collection_ignores_configured_runtime_credential(
    ...
    monkeypatch.setattr(
        "nico.snapshot_repository_evidence.GitHubAssessmentClient",
        lambda: configured,
    )
    monkeypatch.setattr(
        "nico.snapshot_repository_evidence._public_git_profile",
        lambda *_args: (fallback_profile, ""),
    )

    repository, _ = collect_snapshot_repository_evidence(context, snapshot)

    assert configured.access_mode == "anonymous_public"
    assert configured.credential_used is False
    assert not any(
        str(key).casefold() == "authorization" for key in configured.headers
    )
    assert repository["provider_access_mode"] == "anonymous_public"
    assert repository["provider_credential_used"] is False
    assert repository["required_source_evidence_complete"] is True
