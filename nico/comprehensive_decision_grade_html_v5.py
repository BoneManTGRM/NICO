...
def _build_html(identity: dict[str, Any], assessment: dict[str, Any], stages: list[dict[str, Any]], roadmap: list[dict[str, Any]], staffing: list[dict[str, Any]], limitations: dict[str, int], generated_at: str) -> str:
    ...
    risk_rows = [[item.get("priority"), item.get("title"), item.get("impact"), item.get("confidence"), item.get("recommendation")] for item in executive_risks]
    weight_rows = [[item.get("control"), item.get("weight_display"), item.get("score") if item.get("score") is not None else "—", item.get("weighted_contribution") if item.get("weighted_contribution") is not None else "—", "Yes" if item.get("included") else "No", item.get("assurance")] for item in weights]
    finding_cards = "".join(
        f"<section class='finding'><h3>{html.escape(_text(item.get('priority')))} · {html.escape(_text(item.get('title')))}</h3>"
        f"<dl><dt>Candidate status</dt><dd>{html.escape(_text(item.get('confidence')))}</dd><dt>Location</dt><dd>{html.escape(_text(item.get('location')))}</dd><dt>Evidence</dt><dd>{html.escape(_text(item.get('evidence')))}</dd>"
    ...
    roadmap_html = "".join(
        ...
        + "".join(
            ...
            for package in window.get("work_packages") or [] if isinstance(package, dict)
        )
        + "</section>"
        for window in roadmap if isinstance(window, dict)
    )
    stage_html = "".join(
        ...
        + ("<ul>" + "".join(f"<li>{html.escape(item)}</li>" for item in (stage.get("evidence") or [])[:6]) + "</ul>" if stage.get("evidence") else "")
        + "</section>"
...
<section><h2>Assessment Dashboard</h2>{_html_table(['Dimension','Result','Meaning'], [['Technical maturity', f"{maturity.get('score_band_label') or _score_band(score)['score_band_label']} · {int(score)}/100" if isinstance(score,(int,float)) else 'NOT SCORED','Weighted engineering health; unscored controls are excluded rather than treated as zero'],['Evidence readiness',maturity.get('evidence_readiness_score') or 'Pending','Completeness and reliability of required evidence'],['Human review','REQUIRED','Findings and assumptions remain human responsibilities'],['Client delivery','NOT AUTHORIZED','Approval required for the exact package']])}</section>
...
<section><h2>Weighted Maturity Calculation</h2>{_html_table(['Control','Weight','Score','Contribution','Included','Assurance'],weight_rows)}</section>
<section><h2>Executive Risk Register</h2><p>Consolidated decision risks only. Exact instances remain in the technical register.</p>{_html_table(['Priority','Consolidated risk','Business impact','Confidence','Recommended action'],risk_rows or [['—','No consolidated risk retained','Human review remains required','—','Verify evidence completeness']])}</section>
<h2>Technical Findings Register</h2>{finding_cards}
