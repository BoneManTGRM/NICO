...
def _payload_for_result(result: dict[str, Any]) -> dict[str, Any]:
    return {
        "repository": _repo(result),
        "authorized": True,
        "authorized_by": "hosted-final-truth-gate",
        "authorization_scope": "same authorized assessment request; server-side scanner evidence refresh",
...
def _attach_raw_artifact(result: dict[str, Any], artifact: dict[str, Any]) -> dict[str, Any]:
    ...
    result.setdefault("report_quality_guards", {})["hosted_full_evidence_runtime"] = {
        ...
        "guardrail": "Runtime evidence refresh attaches only scanner output produced by the hosted worker; missing tools remain unavailable.",
    }
...
def ensure_hosted_runtime_evidence(result: dict[str, Any]) -> dict[str, Any]:
    """Run the hosted scanner once when yellow sections lack runtime artifacts.

    This is an evidence collection step, not a score override. Any unavailable tool
    or scanner finding remains visible to the later scanner artifact integration,
    trust engine, ledger, and export truth gate.
    """
    ...
    if not isinstance(artifact, dict):
        result.setdefault("report_quality_guards", {})["hosted_full_evidence_runtime"] = {
            "status": "failed",
            "guardrail": "Hosted runtime evidence refresh did not return an artifact.",
        }
        return result
