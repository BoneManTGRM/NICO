...
from nico.full_assessment_idempotency import matching_report
...
def build_report_package(
    ...
    *,
    ...
    payload = _guard_payload(payload)
    run_id = str(payload.get("run_id") or payload.get("assessment_id") or "").strip()
    resolved_report_id = str(report_id or f"report_{uuid4().hex[:16]}")

    if report_id and idempotency_key:
        existing = STORE.get("reports", resolved_report_id)
        if existing:
            if not matching_report(existing, run_id=run_id, idempotency_key=idempotency_key):
                raise ValueError("report idempotency identity conflicts with an existing report record")
    ...
    package = {
        ...
        "run_id": run_id or resolved_report_id,
        ...
        "unavailable_data_notes": ["PDF export is added only by a report-path-specific renderer after truth-gate validation; this initial package stores Markdown, HTML, and JSON."],
        "created_at": now_iso(),
    }
    STORE.put("reports", resolved_report_id, package)
    STORE.audit("report.created", {"report_id": resolved_report_id, "run_id": package["run_id"], "idempotency_key": idempotency_key}, customer_id=package["customer_id"], project_id=package["project_id"])
    return package
