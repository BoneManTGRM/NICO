...
def _evidence_health(contract: DecisionGradeContract) -> dict[str, Any]:
    ...
    return {
        ...
        "required_scanner_failures": [
            item.scanner_name
            for item in contract.scanner_executions
            if item.required and item.status.value in incomplete
        ],
        "confidence_effect": (
            "Required evidence is incomplete; the Evidence-Adjusted score and delivery posture must remain constrained."
            if any(item.required and item.status.value in incomplete for item in contract.scanner_executions)
            else "No required scanner failure was retained in the structured execution records."
        ),
    }
