...
from nico.comprehensive_decision_grade_assessment_v5 import (
    build_decision_grade_assessment,
...
from nico.decision_grade_premium_delivery_v1 import (
    VERSION as DECISION_GRADE_PREMIUM_DELIVERY_VERSION,
    wrap_report_builder_with_premium_delivery_package,
)
from nico.decision_grade_supply_chain_binding_v1 import (
    VERSION as DECISION_GRADE_SUPPLY_CHAIN_BINDING_VERSION,
    wrap_report_builder_with_supply_chain,
)
...
_SCAN_DETAILS: ContextVar[dict[str, Any] | None] = ContextVar(
    "nico_v5_scan_details",
    default=None,
)
...
def _safe_sample_wrapper(
    delegate: Callable[[dict[str, str]], dict[str, Any]],
...
def _quality_report_builder(
    ...
    def wrapped(*args: Any, **kwargs: Any) -> dict[str, Any]:
        ...
        if isinstance(assessment, dict):
            ...
            package = (
                result.get("report_package")
                if isinstance(result.get("report_package"), dict)
                else {}
            )
            package["technical_score"] = normalized.get("technical_score")
            package["evidence_adjusted_score"] = normalized.get(
                "canonical_evidence_adjusted_score"
            )
            package["canonical_evidence_adjusted_score"] = normalized.get(
                "canonical_evidence_adjusted_score"
            )
...
def install_decision_grade_binding() -> dict[str, Any]:
    ...
    evidence_quality_provider = wrap_evidence_quality_provider(
        canonical_scoring_provider
    )
    score_truth_provider = wrap_scoring_provider(evidence_quality_provider)
    history_builder = wrap_report_builder_with_persisted_history(
        report_module.build_comprehensive_report_package
    )
    ...
    ci_reliability_builder = wrap_report_builder_with_ci_reliability(
        human_evidence_builder
    )
    supply_chain_builder = wrap_report_builder_with_supply_chain(
        ci_reliability_builder
    )
    premium_delivery_builder = wrap_report_builder_with_premium_delivery_package(
        supply_chain_builder
    )
    build_comprehensive_report_package = wrap_report_builder(
        premium_delivery_builder
    )
    ...
    return {
        ...
        "bound": providers.build_comprehensive_report_package
        is build_comprehensive_report_package,
        "canonical_scoring_bound": providers.canonical_scoring_provider
        is score_truth_provider,
        "repository_evidence_samples_bound": providers.collect_snapshot_repository_evidence
        is _collect_with_safe_samples,
        ...
        "full_source_profile_bound": full_source_profile.get("status")
        in {"installed", "already_installed"},
        "typescript_ast_complexity_bound": typescript_ast_complexity.get("status")
        in {"installed", "already_installed"},
        ...
        "report_clarity_bound": report_clarity.get("status")
        in {"installed", "already_installed"},
        "report_polish_bound": report_polish.get("status")
        in {"installed", "already_installed"},
        "code_remediation_bound": code_remediation.get("status")
        in {"installed", "already_installed"},
        "code_remediation_outline_bound": code_outline.get("status")
        in {"installed", "already_installed"},
        "final_pdf_front_matter_bound": final_pdf_front_matter.get("status")
        in {"installed", "already_installed"},
        "executive_brief_page_gate_bound": executive_brief_page_gate.get("status")
        in {"installed", "already_installed"},
        ...
        "decision_grade_cost_engine_bound": cost_of_inaction_engine.get("bound")
        is True,
        ...
        "decision_grade_acceptance_version": DECISION_GRADE_ACCEPTANCE_VERSION,
        "decision_grade_acceptance_bound": acceptance_engine.get("bound") is True,
        "deterministic_acceptance_templates_enabled": True,
        "contextual_acceptance_drafts_validated": True,
        "p0_p1_multi_criterion_coverage_required": True,
        "binary_acceptance_results_required": True,
        ...
        "decision_grade_supply_chain_binding_version": DECISION_GRADE_SUPPLY_CHAIN_BINDING_VERSION,
        "decision_grade_supply_chain_bound": True,
        "decision_grade_premium_delivery_version": DECISION_GRADE_PREMIUM_DELIVERY_VERSION,
        "decision_grade_premium_delivery_bound": True,
        "deterministic_delivery_zip_generated": True,
        "executive_detailed_and_appendix_pdfs_separated": True,
        "delivery_manifest_hashes_every_artifact": True,
        "unapproved_delivery_package_is_internal_only": True,
        "exact_location_code_remediation_plan": code_remediation.get(
            "exact_location_code_plan"
        )
        is True,
        "pdf_code_remediation_appendix": code_remediation.get("pdf_code_pages")
        is True,
        "pdf_outline_preserved_after_appendix": code_outline.get(
            "base_outline_preserved"
        )
        is True,
        "front_matter_pages_replaced_not_overlaid": final_pdf_front_matter.get(
            "front_matter_pages_replaced_not_overlaid"
        )
        is True,
        "unverified_candidates_not_p1": report_polish.get(
            "unverified_candidates_not_p1"
        )
        is True,
        "equivalent_review_candidates_grouped": report_polish.get(
            "equivalent_review_candidates_grouped"
        )
        is True,
...
__all__ = [
    ...
    "build_roadmap",
    "canonical_scoring_provider",
    "install_decision_grade_binding",
]
