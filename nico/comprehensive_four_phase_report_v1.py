...
from nico.comprehensive_four_phase_pdf_v1 import (
    apply_four_phase_pdf,
    assert_four_phase_pdf,
)

_ATTACH_MARKER = "__nico_comprehensive_four_phase_manifest_input_v1__"
_APPEND_MARKER = "__nico_comprehensive_four_phase_terminal_pdf_v1__"
_PDF_CONTEXT: ContextVar[dict[str, Any]] = ContextVar(
    "nico_comprehensive_four_phase_terminal_pdf_context",
    default={},
)
...
def _synchronize_canonical_json_artifact_digest(
    ...
    """Keep earlier canonical projections bound to final four-phase truth."""
...
def finalize_four_phase_report_package(
    package: Mapping[str, Any],
    *,
    publish_pdf: bool = True,
) -> dict[str, Any]:
    """Publish phase truth, optionally deferring PDF work to the terminal compositor."""
    ...
    source_pdf = base64.b64decode(encoded)
    pdf = (
        apply_four_phase_pdf(source_pdf, canonical, spanish=spanish)
        if publish_pdf
        else source_pdf
    )
    ...
    pdf_proof: dict[str, Any] | None = None
    if publish_pdf:
        pdf_proof = assert_four_phase_pdf(pdf, canonical, spanish=spanish)
        surfaces.append(
    ...
    result.update(
        {
            ...
            "client_delivery_allowed": (
                canonical.get("client_delivery_allowed") is True
            ),
        }
    )
    ...
    completion.update(
        {
            ...
            "four_phase_program_in_pdf": publish_pdf,
            "four_phase_toc_matrix_present": publish_pdf,
            "four_phase_pdf_bookmarks_present": publish_pdf,
            "four_phase_pdf_publication_deferred": not publish_pdf,
            "four_phase_toc_page_number": (
                pdf_proof["target_page_number"] if pdf_proof else None
            ),
            "all_four_phases_present": publish_pdf,
...
def _terminalize_completion(
    result: Mapping[str, Any],
    proof: Mapping[str, Any],
) -> dict[str, Any]:
    output = deepcopy(dict(result))
    completion = _copy(output.get("client_report_completion"))
    completion.update(
        {
            "four_phase_report_version": VERSION,
            "four_phase_program_in_json": True,
            ...
            "four_phase_pdf_bookmarks_present": True,
            "four_phase_pdf_publication_deferred": False,
            "four_phase_terminal_pdf_publication": True,
            "four_phase_toc_page_number": proof.get("target_page_number"),
...
def install_comprehensive_four_phase_report_v1() -> dict[str, Any]:
    """Bind phase publication inside the final pre-digest PDF compositor."""

    from nico import comprehensive_artifact_manifest_approval_v1 as manifest

    append_installed = False
    current_append: Callable[[bytes, bytes], bytes] = manifest._append_pdf
    if not getattr(current_append, _APPEND_MARKER, False):

        @wraps(current_append)
        def _append_pdf(base_pdf: bytes, supplement: bytes) -> bytes:
            combined = current_append(base_pdf, supplement)
            context = _PDF_CONTEXT.get()
            canonical = (
                context.get("json")
                if isinstance(context.get("json"), Mapping)
                else {}
            )
            if not canonical:
                raise RuntimeError(
                    "four-phase terminal PDF publication has no canonical context"
                )
            spanish = _spanish(canonical)
        ...
        setattr(_append_pdf, _APPEND_MARKER, True)
        setattr(_append_pdf, "_nico_previous", current_append)
        manifest._append_pdf = _append_pdf
        append_installed = True

    attach_installed = False
    current_attach: Callable[..., dict[str, Any]] = manifest.attach_artifact_manifest
    if not getattr(current_attach, _ATTACH_MARKER, False):
        ...
        def attach_artifact_manifest(
            ...
            prepared = finalize_four_phase_report_package(
                package,
                publish_pdf=False,
            )
            token = _PDF_CONTEXT.set(prepared)
            try:
                result = current_attach(prepared)
            finally:
                _PDF_CONTEXT.reset(token)
            canonical = (
                result.get("json")
            ...
            proof = assert_four_phase_pdf(
                pdf,
                canonical,
                spanish=_spanish(canonical),
            )
            return _terminalize_completion(result, proof)
        ...
        attach_installed = True

    append_bound = getattr(manifest._append_pdf, _APPEND_MARKER, False)
    attach_bound = getattr(
        manifest.attach_artifact_manifest,
        _ATTACH_MARKER,
        False,
    )
    return {
        "status": (
            "installed"
            if append_installed or attach_installed
            else "already_installed"
        ),
        "version": VERSION,
        "bound": bool(append_bound and attach_bound),
        "terminal_pdf_append_bound": bool(append_bound),
        "manifest_input_bound": bool(attach_bound),
        "phase_count": 4,
        ...
        "terminal_pdf_publication_before_digest_binding": True,
