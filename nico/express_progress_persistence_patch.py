...
_STAGE_ORDER = {"request_accepted": 0, "repository_evidence": 1, "scanner_reconciliation": 2, "accuracy_review": 3, "score_reconciliation": 4, "report_generation": 5, "truth_and_review_gates": 6, "complete": 7, "blocked": 7, "failed": 7, "interrupted": 7}
...
def _progress_identity(response: dict[str, Any]) -> dict[str, Any]:
    return {"run_id": str(response.get("run_id") or ""), "status": str(response.get("status") or "unknown"), "current_stage": str(response.get("current_stage") or "request_accepted"), "progress_percent": max(0, min(100, int(response.get("progress_percent") or 0))), "progress": deepcopy(response.get("progress") or []), "updated_at": response.get("updated_at"), "human_review_required": True, "client_ready": False}
...
def _overlay_progress(api: Any, run_id: str, response: dict[str, Any]) -> dict[str, Any]:
    ...
    if status in _TERMINAL_SUCCESS:
        ...
        output["progress"] = api._stage_progress("complete", "complete", "Express assessment completed. Draft report artifacts are ready for required human review.", evidence={"report_formats_ready": bool((output.get("reports") or {}).get("pdf_base64"))})
    ...
    use_stored = (stored_status in {"queued", "running"} and _STAGE_ORDER.get(stored_stage, -1) >= _STAGE_ORDER.get(response_stage, -1)) or stored_status in _TERMINAL_FAILURE
    if not use_stored:
        return output
...
def install_express_progress_persistence() -> dict[str, Any]:
    ...
    if not getattr(current_record, _RECORD_MARKER, False):
        def record_with_independent_progress(run_id: str, request_payload: dict[str, Any], response: dict[str, Any]) -> dict[str, Any]:
    ...
    if not getattr(current_status, _STATUS_MARKER, False):
        def status_with_independent_progress(run_id: str, req: Any) -> dict[str, Any]:
            return _overlay_progress(api, run_id, current_status(run_id, req))
    ...
    return {"status": "installed", "version": EXPRESS_PROGRESS_PERSISTENCE_VERSION, "collection": _PROGRESS_COLLECTION, "report_record_overwrite_can_reset_progress": False, "terminal_success_forces_100_percent": True, "tenant_scope_failures_are_never_overlaid": True, "repository_stage_watchdog": watchdog, "human_review_required": True, "client_ready": False}


__all__ = ["EXPRESS_PROGRESS_PERSISTENCE_VERSION", "install_express_progress_persistence"]
