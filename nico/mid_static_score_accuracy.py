...
def _verified_tools(scanner: dict[str, Any]) -> set[str]:
    """Return only completed current-run tools whose result did not fail verification.

    Older retained evidence may not contain per-tool result records. In that case,
    the already-normalized tools_run list remains the bounded fallback.
    """
...
def _category_counts(scanner: dict[str, Any], category: str) -> dict[str, int]:
    ...
    # Legacy scanner summaries exposed one raw category count plus severity maps.
    raw = _int(value)
...
def apply_verified_control_reconciliation(
    ...
    """Reconcile Mid controls with verified exact-snapshot evidence.

    The original shared scorecard treats sampled pattern matches as if they were
    confirmed defects and does not fully credit all Mid analyzers. This bounded
    Mid-only pass corrects that mismatch. It never raises a category with a
    material scanner finding, never removes findings, and never treats missing or
    failed tools as clean evidence.
    """

    output = deepcopy(assessment)
    if repository_evidence.get("status") != "attached" or scanner_evidence.get("status") != "attached":
        return output
    if scanner_evidence.get("snapshot_match") is not True:
        return output
    ...
    if code is not None and static_counts["material"] == 0:
        files_profiled = _int(files.get("files_profiled"))
        source_count = _int(architecture.get("source_file_count"))
        test_count = _int(architecture.get("test_path_count"))
        docs = _int(architecture.get("documentation_path_count"))
        commits = _int(activity.get("commits_returned"))
        pulls = _int(activity.get("pull_requests_returned"))
        risks = _int(signals.get("risk_pattern_hits"))
        todos = _int(signals.get("todo_fixme_security_notes"))
        candidate = 36
        candidate += 10 if files_profiled >= 10 else 5 if files_profiled else 0
        candidate += 10 if source_count >= 5 else 5 if source_count else 0
        candidate += 12 if test_count >= 3 else 6 if test_count else 0
        candidate += 6 if docs else 0
        candidate += 5 if commits else 0
        candidate += 5 if pulls else 0
        candidate += min(6, len(verified_static) * 2)
        candidate -= min(6, risks)
        candidate -= min(4, todos)
        candidate -= len(failed & STATIC_TOOLS) * 6
        candidate -= len(timed_out & STATIC_TOOLS) * 4
        candidate -= len(unavailable & STATIC_TOOLS)
        previous, revised = _apply_candidate(
            code,
            candidate=candidate,
            evidence_note=(
    ...
    if dependency is not None and dependency_counts["material"] == 0:
        manifests = _list(dependencies.get("manifest_paths"))
        lockfiles = _list(dependencies.get("lockfile_paths"))
        entries = _int(dependencies.get("dependency_entries"))
        candidate = 38
        candidate += 14 if manifests else 0
        candidate += 14 if lockfiles else 0
        candidate += 10 if entries else 0
    ...
    if static is not None and static_counts["material"] == 0:
        risks = _int(signals.get("risk_pattern_hits"))
        candidate = 52
        candidate += min(32, len(verified_static) * 8)
    ...
    maturity.update(
        {
            "score": technical_score,
            "level": _maturity_level(technical_score),
            "summary": "Weighted Mid technical score reconciled to verified exact-snapshot repository and analyzer evidence; material findings, failed tools, and missing evidence remain adverse.",
        }
    )
    scorecard = deepcopy(_dict(output.get("scorecard")))
    scorecard["technical_score"] = technical_score
...
def mid_scoring_handler(context: dict[str, Any], outputs: dict[str, Any]) -> dict[str, Any]:
    """Run the shared scorecard, then apply Mid-only verified evidence accuracy."""
