...
from nico.decision_grade_supply_chain_v2 import VERSION as SUPPLY_CHAIN_VERSION, build_supply_chain_package

VERSION = "nico.decision_grade_supply_chain_binding.v1"
_MARKER = "__nico_decision_grade_supply_chain_binding_v1__"
...
def _source_files(args: tuple[Any, ...], kwargs: dict[str, Any]) -> dict[str, str]:
    ...
    if isinstance(stages, dict):
        for stage_name in ("repository_and_delivery_evidence", "immutable_repository_snapshot"):
            stage = stages.get(stage_name)
            if not isinstance(stage, dict):
                continue
            for key in ("source_files", "repository_files", "files", "snapshot_files"):
                value = stage.get(key)
                if isinstance(value, dict) and all(isinstance(path, str) for path in value):
                    return {str(path): str(content) for path, content in value.items()}
    return {}
...
def _identity(args: tuple[Any, ...], kwargs: dict[str, Any], result: dict[str, Any]) -> tuple[str, str]:
    repository = str(kwargs.get("repository") or result.get("repository") or "")
    commit_sha = str(kwargs.get("commit_sha") or result.get("commit_sha") or "")
    return repository, commit_sha


def wrap_report_builder_with_supply_chain(delegate: Callable[..., dict[str, Any]]) -> Callable[..., dict[str, Any]]:
    ...
    def wrapped(*args: Any, **kwargs: Any) -> dict[str, Any]:
        ...
        if files:
            evidence = build_supply_chain_package(files, repository=repository, commit_sha=commit_sha)
            status = "complete"
        else:
            evidence = {
