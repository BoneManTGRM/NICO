...
def _invoke(operation, authority, body):
    ...
    if operation == "claim":
        ...
        elif operation == "artifact" and set(body) == {"lease_id", "artifact"}:
            reference = jobs.put_artifact(identity, lease, authority.worker_id, body["artifact"])
            result = {**job, "artifact": reference}
        else:
            raise ValueError("worker_request_invalid")
    ...
    return {key: result[key] for key in ("job_id", "identity", "contract", "limits", "status",
        ...
        "worker_id", "source_access", "artifact") if key in result}


async def _endpoint(request: Request, job_id: str, operation: str):
    if not JOB_ID.fullmatch(job_id) or operation not in {"claim", "heartbeat", "receipt", "artifact", "fail"}:
...
async def _authenticated_request(request, job_id, operation, token):
    ...
    limit = (MAX_ARTIFACT_REQUEST_BYTES if operation == "artifact" else MAX_REQUEST_BYTES if operation == "receipt" else 4096)
