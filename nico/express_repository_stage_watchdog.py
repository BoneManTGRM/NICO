...
def _run_with_watchdog(api: Any, function: Callable[[dict[str, Any]], dict[str, Any]], payload: dict[str, Any]) -> dict[str, Any]:
    context = _context()
    if context is None:
...
def install_express_repository_stage_watchdog() -> dict[str, Any]:
    ...
    for name in ("run_github_assessment", "run_github_assessment_with_scanner_artifacts"):
        ...
        def watched(payload: dict[str, Any], _current: Callable[[dict[str, Any]], dict[str, Any]] = current) -> dict[str, Any]:
