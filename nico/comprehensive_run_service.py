...
from nico.comprehensive_background_terminal_order_v2 import install_background_terminal_ordering
from nico.comprehensive_blocked_run_recovery_v1 import rewind_blocked_run_for_final_artifact_recovery
...
from nico.comprehensive_pre_render_scanner_truth_v65 import install_pre_render_authoritative_scanner_truth
...
from nico.comprehensive_stage_watchdog_v1 import apply_stage_watchdog, rewind_stalled_stage_for_retry
...
class ComprehensiveRunService:
    ...
    delivery archive is generated only after the accepted-edition and client-readiness
    manifests validate.
    """
    ...
    def resume(self, run_id: str, *, max_stages: int | None = None) -> dict[str, Any]:
        ...
        if record.get("terminal"):
            ...
            if recovered == record:
                return record
            record = self._store.save(recovered, expected_revision=int(record["revision"]))
    ...
    def review(
        ...
        *,
        ...
        updated = apply_comprehensive_review_decision(record, manifest=manifest)
    ...
    def _run_next_stage(self, record: dict[str, Any]) -> dict[str, Any]:
        ...
        if executor is None:
            ...
            retained_stage_results = record.get("stage_results") if isinstance(record.get("stage_results"), dict) else {}
            context = {
                ...
                "prior_stage_results": retained_stage_results if stage_id == FINAL_REPORT_STAGE_ID else deepcopy(retained_stage_results),
            ...
            if stage_id == FINAL_REPORT_STAGE_ID:
                ...
                raw = execute_background_stage(executor, context, stage_id=stage_id)
                result = raw if is_background_stage_in_progress(raw) else apply_stage_watchdog(
                    record,
                    stage_id=stage_id,
                    result=raw,
                )
            else:
                raw = execute_stage_with_timeout(executor, context, stage_id=stage_id)
                result = apply_stage_watchdog(record, stage_id=stage_id, result=raw)

        previous_revision = int(record["revision"])
        updated = apply_comprehensive_stage_result(record, stage_id=stage_id, result=result)
        return self._store.save(updated, expected_revision=previous_revision)
