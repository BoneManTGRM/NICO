...
def build_report_evidence_status(result: dict[str, Any]) -> dict[str, Any]:
    ...
    osv_with_findings = "osv returned" in dependency.lower() and "vulnerability record" in dependency.lower()
    secret_clean_artifacts = (
        "credential-scan" in secrets.lower()
        and ("zero high-confidence" in secrets.lower() or "clean credential-scan" in secrets.lower())
    )
    full_history_missing = "full git-history" in secrets.lower() and (
        "not verified" in secrets.lower()
        or "requires" in secrets.lower()
        or "unavailable" in secrets.lower()
        or "missing" in secrets.lower()
    )
    ...
    return {
        ...
        "capabilities": {
            "scanner_worker": {
                "status": "built_but_runtime_artifact_missing" if scanner_worker_built else "not_built",
                "built": scanner_worker_built,
            },
            "github_app_private_repo_flow": {
                "status": "built_but_runtime_artifact_missing" if github_app_built else "not_built",
                "built": github_app_built,
            },
            "mid_modules": {
                "status": "built_but_runtime_artifact_missing" if mid_built else "not_built",
                "built": mid_built,
            },
            "retainer_modules": {
                "status": "built_but_runtime_artifact_missing" if retainer_built else "not_built",
                "built": retainer_built,
            },
            "report_readiness_gate": {
                "status": "built_but_runtime_artifact_missing" if readiness_built else "not_built",
                "built": readiness_built,
            },
            "report_readiness_attachment": {
                "status": "built_but_runtime_artifact_missing" if readiness_attach_built else "not_built",
                "built": readiness_attach_built,
            },
        },
        "dependency_tools": {
            ...
            "OSV API": {
                "tool": "OSV API",
                "status": "completed_with_findings" if osv_with_findings else ("completed_clean" if "osv returned no vulnerability records" in dependency.lower() else "unavailable_in_this_run"),
                "built": True,
                "wired": True,
                "human_review_required": osv_with_findings,
            },
        },
        "secret_tools": {
            "credential-scan": {
                "tool": "credential-scan",
                "status": "completed_clean" if secret_clean_artifacts else "unavailable_in_this_run",
                "built": True,
                "wired": True,
                "human_review_required": False,
            },
            "gitleaks": _classify_tool(
                built=scanner_worker_built,
                text=secrets,
                tool="gitleaks",
                complete_without_findings="gitleaks" in secrets.lower() and secret_clean_artifacts,
            ),
            "trufflehog": _classify_tool(built=scanner_worker_built, text=secrets, tool="trufflehog"),
            "full_git_history_coverage": {
                "tool": "full_git_history_coverage",
                "status": "built_but_runtime_artifact_missing" if full_history_missing else "completed_clean",
                "built": scanner_worker_built,
                "wired": True,
                "human_review_required": full_history_missing,
            },
        },
        "static_tools": {
        ...
        "readiness_gates": {
            "deployment_verification": {
                "status": "completed_clean" if result.get("deployment_verification") else "built_but_runtime_artifact_missing",
                "built": _module_exists("nico.deployment_verification"),
            },
            "hosted_smoke_test": {
                "status": "completed_clean" if result.get("hosted_smoke_test") else "built_but_runtime_artifact_missing",
                "built": _module_exists("nico.hosted_smoke_test"),
            },
            "report_readiness_gate": {
                "status": "completed_clean" if result.get("report_readiness_gate") else "built_but_runtime_artifact_missing",
                "built": readiness_built,
            },
            "client_human_acceptance": {
                "status": "completed_clean" if result.get("client_acceptance", {}).get("status") in {"accepted", "approved"} else "needs_human_review",
                "built": _module_exists("nico.client_acceptance"),
            },
        },
    }
...
def _apply_dependency_language(result: dict[str, Any]) -> None:
    ...
    if "osv returned" in text and "vulnerability record" in text:
        section["summary"] = (
        ...
        section["summary"] = (
            "Dependency review uses attached audit artifacts and available manifest evidence; missing tools remain disclosed separately."
        )
...
def _apply_secret_language(result: dict[str, Any]) -> None:
    ...
    history_gap = "full git-history" in text and ("not verified" in text or "requires" in text or "unavailable" in text)
    if clean_artifacts and history_gap:
        ...
        if "scanner-worker secret tools unavailable" in text and "gitleaks" in text:
            _append_unique(
                section["unavailable"],
                "Secret scanner source distinction: attached clean credential/gitleaks artifact evidence is separate from live scanner-worker gitleaks/trufflehog execution and full-history coverage for this run.",
            )
...
def _apply_static_language(result: dict[str, Any]) -> None:
    ...
    bandit_total = _bandit_count(text)
    if bandit_total:
        ...
        review_count = int(triage.get("review_required_count") or 0)
        false_positive_count = int(triage.get("candidate_false_positive_count") or 0)
        if not triage:
            review_count = bandit_total
        section["summary"] = (
        ...
        _append_unique(
            section["evidence"],
            "Bandit evidence source distinction: parsed prior/sample or attached artifact findings are separate from live scanner-worker Bandit execution for this report run.",
        )
        _append_unique(
            section["findings"],
            f"Bandit triage summary: total={bandit_total}, blocker_count={blocker_count}, review_required_count={review_count}, candidate_false_positive_count={false_positive_count}; score impact=needs_human_review until rule-level triage is attached and approved.",
        )
        if "scanner-worker static tools unavailable" in lower or "bandit" in lower and "unavailable" in lower:
            _append_unique(
                section["unavailable"],
                "Bandit source distinction: Bandit findings were parsed from available artifact text, but live scanner-worker Bandit execution is not verified for this report run.",
            )
...
def _dynamic_medium_term_plan(status: dict[str, Any]) -> list[str]:
    ...
    plan: list[str] = []
    if scanner_built:
        plan.append("Run and attach verified scanner-worker artifacts for dependency, secret, static-analysis, coverage, and complexity evidence in each client-facing report.")
    else:
        plan.append("Build scanner-worker execution for dependency, secret, static-analysis, coverage, and complexity evidence.")

    if github_built:
        plan.append("Maintain and verify GitHub App/private-repository authorization evidence in hosted report runs without exposing credentials.")
    else:
        plan.append("Build authenticated GitHub App/private-repository authorization before private-repo client delivery.")

    if mid_built:
        plan.append("Maintain Mid assessment modules and attach verified QA/parity/stakeholder/roadmap evidence when the selected workflow requires them.")
    else:
        plan.append("Build Mid assessment modules for QA/parity/stakeholder/roadmap evidence.")

    if retainer_built:
        plan.append("Maintain Retainer Ops modules and attach verified weekly/monthly/release evidence when a retainer workflow is selected.")
    else:
        plan.append("Build Retainer Ops modules for weekly/monthly/release evidence.")

    if readiness_built:
        plan.append("Run and attach deployment, smoke-test, report-readiness, delivery-manifest, final-review, and client-acceptance artifacts before client-facing delivery.")
    else:
        plan.append("Build report readiness and delivery-manifest gates before client-facing delivery.")

    return plan


def apply_report_evidence_status(result: dict[str, Any]) -> dict[str, Any]:
    """Attach current evidence-state classification and repair stale report wording.

    This does not hide unavailable evidence or change scores. It separates built
    capability from runtime proof attached to the specific report run.
    """
