...
def _approval_error(result: dict[str, Any], default_message: str) -> None:
    ...
    if result.get("status") == "blocked":
        raise HTTPException(
            ...
            detail={"status": "blocked", "message": str(result.get("error") or default_message), "validation": result.get("validation") or {}},
        )


def mid_approval_request_response(
    run_id: str,
    req: MidApprovalRequest,
    x_nico_admin_token: str = Header(default=""),
) -> dict[str, Any]:
    result = request_mid_approval(
        run_id,
        customer_id=req.customer_id,
        project_id=req.project_id,
        admin_token=x_nico_admin_token,
    )
    _approval_error(result, "Mid approval request was unavailable.")
    return result
...
def mid_approval_status_response(
    ...
    result = mid_approval_status(
        run_id,
        customer_id=customer_id,
        project_id=project_id,
        admin_token=x_nico_admin_token,
    )
    _approval_error(result, "Mid Assessment run not found.")
    return result
...
def mid_approval_decision_response(
    ...
    _approval_error(result, "Mid approval not found.")
    return result


def _public_approved_report(report: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "rule": "This is a separate human-approved Mid artifact. Secure client delivery remains disabled until the dedicated delivery workflow verifies and grants access.",
    }
...
def mid_approved_report_response(
    ...
    result = get_mid_approved_report(
        run_id,
        customer_id=customer_id,
        project_id=project_id,
        admin_token=x_nico_admin_token,
    )
    _approval_error(result, "Approved Mid report not found.")
    return _public_approved_report(result)
...
def mid_approved_report_pdf_response(
    ...
    report = get_mid_approved_report(
        run_id,
        customer_id=customer_id,
        project_id=project_id,
        admin_token=x_nico_admin_token,
    )
    _approval_error(report, "Approved Mid report not found.")
    formats = report.get("formats") if isinstance(report.get("formats"), dict) else {}
    try:
    ...
    exposed = ", ".join(
        [
            ...
            "X-NICO-Source-Draft-PDF-SHA256",
...
__all__ = [
    ...
    "mid_approval_status_response",
    "mid_approval_decision_response",
    "mid_approved_report_response",
    "mid_approved_report_pdf_response",
    "register_mid_approval_routes",
]
