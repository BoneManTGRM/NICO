...
CANONICAL_EXPRESS_CONTRACT = install_canonical_express_contract_binding_v1()
...
app.state.nico_canonical_express_contract = CANONICAL_EXPRESS_CONTRACT
...
if CANONICAL_EXPRESS_CONTRACT.get("status") not in {"installed", "already_installed"}:
    raise RuntimeError(f"Canonical Express contract did not install: {CANONICAL_EXPRESS_CONTRACT}")
if CANONICAL_EXPRESS_CONTRACT.get("completed_express_runs_receive_core_contract") is not True:
    raise RuntimeError("Completed Express runs do not receive the canonical Core contract")
if CANONICAL_EXPRESS_CONTRACT.get("same_contract_used_by_strategic") is not True:
    raise RuntimeError("Core and Strategic are not bound to the same canonical assessment contract")
if CANONICAL_EXPRESS_CONTRACT.get("independent_core_scorecard_allowed") is not False:
    raise RuntimeError("The Express boundary still permits an independent Core scorecard")
if CANONICAL_EXPRESS_CONTRACT.get("human_review_required") is not True:
    raise RuntimeError("Canonical Core contract binding must preserve required human review")
if CANONICAL_EXPRESS_CONTRACT.get("client_delivery_allowed") is not False:
    raise RuntimeError("Canonical Core contract binding must block client delivery")
