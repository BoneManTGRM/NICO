...
def _render_pdf(report: dict[str, Any], approval: dict[str, Any], identity: dict[str, Any]) -> bytes:
    ...
    bullet("Approval does not create a client link or record a client delivery receipt.")
    bullet("Secure delivery remains disabled until the dedicated Mid delivery workflow verifies this approved artifact.")
    bullet("Unsupported claims permitted: 0.")
    heading("Integrity identity")
    paragraph(f"Approval identity SHA-256: {_canonical_hash(identity)}", size=8)
...
def build_mid_approved_report(report: dict[str, Any], approval: dict[str, Any]) -> dict[str, Any]:
    ...
    required = [
        "run_id", "snapshot_id", "snapshot_commit_sha", "source_draft_report_id",
        "source_draft_pdf_sha256", "source_identity_sha256", "review_packet_id",
        "review_packet_sha256", "approval_id", "approved_by", "approved_at",
    ]
